            ...
            for (size_t k=0; k < K-1 ; k++) {
                if (sum(N.col(k)) > 0) {
                    for (size_t kk=k+1; kk < K ; kk++) {
                        if (sum(N.col(kk)) > 0) {
                            double kl_div = KL( xi0.col(k), xi0.col(kk), Omega.slice(k), Omega.slice(kk), alpha.col(k), alpha.col(kk) );
...
void PMC::sampleG(const mat& Y_k, const uvec& C_k, const uvec& N_k, Rcpp::List& particles, mat& log_dQ, const size_t pp) {
    ...
    if (nk==0) {
        ...
        for ( iN=0; iN<num_particles; iN++ ) {
            ...
            g = inv_sympd(rWishartArma(inv_sympd(Lambda_k), ceil(zeta * nk) + m0 ));
            G.row(iN) = vectorise(g).t();
...
void PMC::samplePsi(const mat& Y_k, const uvec& C_k, const uvec& N_k, Rcpp::List& particles, mat& log_dQ, const size_t pp) {
    ...
    if (nk == 0) {
        ...
        for ( int iN=0; iN<num_particles; iN++ ) { 
            ...
            for (int pp=0; pp<p; pp++)  x(pp) = randn(); 
...
// #pragma omp parallel for 
    for (size_t iN=0; iN<num_particles; iN++ ) {
        mat Sigma = reshape(G.row(iN), p, p) + psi.row(iN).t() * psi.row(iN);
        double ld, sgn;
        log_det(ld, sgn, Sigma);
        if (sgn <= 0)  { 
            Rcout << "det(Sigma) is not positive ";
            ...
            mat abs_Sig = abs(Sigma);
            Rcout << "abs(min/max) of Sigma: " << abs_Sig.min() << " " << abs_Sig.max() << " - ";
        }
...
#pragma omp parallel for 
        for (size_t iN=0; iN<num_particles; iN++) {
            ...
            if (sgn <= 0)  Rcout << "det(g) is not positive ";
            g = inv(g);  // save space  
