         ...
         bool init) : Y(Y), psiX(psiX), C(C)
{
    ...
    R = psiX.n_cols;
    // n_re = desr.n_cols;
    // des = arma::join_rows(psiX, desr);
    treestr = Rcpp::as<size_t>(prior["treestr"]);
    to_save_W = Rcpp::as<bool>(prior["to_save_wts"]);
    use_skew = Rcpp::as<bool>(prior["use_skew"]);

    // NMIG_nu = 5;
    // NMIG_Q = 4;
    // ACS_r = 1/10000;

    // Map psiX to unique index
    size_t ncomb = pow(2, R-1);

    // C_small.set_size(J * ncomb);
    CpsiX_small.set_size(J * ncomb, R+1);
    // CpsiX_small.col(1) = 1;
    for (size_t j=0; j<J; j++) {

        for (size_t i=0; i<ncomb; i++) {
            size_t row = j*ncomb + i;

            // C_small(row) = j;
            CpsiX_small(row, 0) = j;
            CpsiX_small(row, 1) = 1;
            // Then set rest of row to base-2 representation (in reverse) of i.
            size_t itmp = i;
            for (size_t col=2; col<R+1; col++) {
                CpsiX_small(row, col) = (itmp % 2);
                itmp /= 2;
            }
        }
    ...
    if (false) {
        Rcout << "CpsiX_small" << endl;
        for (size_t r=0; r<J * ncomb; r++) {
            Rcout << "-----  ";
            for (size_t c=0; c<R+1; c++) {
                Rcout << CpsiX_small(r, c) << " ";
            }
            Rcout << endl;
        }
    }
    ...
    saveWsmall.set_size(J * ncomb, K, length_chain);
    ...
    saveGam.set_size(J, (K-1)*R, length_chain);
    saveGam.fill(0);


    cube gamtest(J, K-1, R);
    for (size_t j=0; j<J; j++)
        for (size_t k=0; k<K-1; k++)
            for (size_t r=0; r<R; r++)
                gamtest(j,k,r) = (j+1)*100 + k;


    mat reshapetest2 = reshape( mat(gamtest.memptr(), gamtest.n_elem, 1, false), J, (K-1)*R);
    if (false) {
        Rcout << "reshapetest2 (J, (K-1)*R): " << endl;
        for (size_t j=0; j<J; j++) {
            for (size_t kr=0; kr<(K-1)*R; kr++)
                Rcout << reshapetest2(j,kr) << " ";
            Rcout << endl;
        }
    }
    ...
    // saveGam.slice(km) = reshape( mat(gam.memptr(), gam.n_elem, 1, false), J, (K-1)*R);
...
void PMC::main_loop(const Rcpp::List& initParticles, bool init) {
    ...
    umat N(J, K);
    mat logW(n, K);
    mat logWsmall(CpsiX_small.n_rows, K);
    // mat logW(J, K);
    // logW.fill(log(1.0/K));
    cube xi(J, p, K);
    mat xi0(p, K);
    ...
    for (size_t j=0; j<J; j++)
        for (size_t m=0; m<(K-1); m++)
            gam.tube(j, m) = tmp.row(j*(K-1) + m);

    // cube effr(J, K-1, n_re);  // XX: Should first dimension be J?
    // cube effall(J, K-1, n_re+R);  // XX: Need to initialize properly.
    ...
    for (size_t it=0; it<(num_iter+num_burnin); it++) {
        ...
        // sampleDelta(delta, gam, SS_om);
        // samplePsiACS(psiACS, gam, delta);

        sampleGam(gam);
        getLogWs(logW, gam);
        // getLogWsSmall(logWsmall, gam);  // for debugging
        ...
        if (to_save_it)
        {
            ...
            getLogWsSmall(logWsmall, gam);
            saveWsmall.slice(km) = exp(logWsmall);
            saveXi.slice(km) = reshape( mat(xi.memptr(), xi.n_elem, 1, false), J, K*p);
...
/*
// Currently not used (can Ctrl-f this function name)
// See two-block Gibbs sampler from Wang and Roy (2018).
void PMC::sampleEff(cube& eff) {

    mat lgam = gam_Sig_inv * gam_mu;
    lgam.resize(R+n_re);
    ...
    for (size_t m=0; m<(K-1); m++) {
        ...
        for (size_t j=0; j<J; j++) {
            ...
            uvec right_of_m;
            if (treestr == 1) {
                under_m = arma::find(lfl<=T && T<(lfl+Km) && C==j);
                right_of_m = arma::find( Tij<(lfl+Km/2) );
            } else {
                under_m = arma::find(T>=m && C==j);
                right_of_m = arma::find( Tij==m );
            }
            uvec Tij = T(under_m);
            mat desij = des.rows(under_m);

            // Block 1. Update precision and PG data
            vec effvec = eff.tube(j, m);
            vec revec = effvec.tail(n_re);
            vec rho = rtgamma(a + 1/2, b + (revec % revec)/2, rho0);
            vec pgdat = rpg1scale(desij * effvec);

            // Block 2. Update fixed and random effects. 
            vec kappa(Tij.size());
            kappa.fill(-0.5);
            kappa(right_of_m).fill(0.5);
            mat Arho = makeArho(rho, gam_Sig_inv);  // XX
            mat eff_Sigjm = arma::inv(desij.t() * diagmat(pgdat) * desij + Arho);
            vec eff_mujm = eff_Sigjm * (desij.t() * kappa + lgam);
            eff.tube(j, m) = mvrnormArma(1, eff_mujm, eff_Sigjm).t();
    ...
    if (treestr == 1) {  // BT
        ...
        for (size_t m=0; m<(K-1); m++) {
            ...
            for (size_t j=0; j<J; j++) {
                ...
                // 1. Update PG data
                vec gamvec = gam.tube(j, m);  // Compiler wants me to store this before multiplying.
                ...
                gam.tube(j, m) = mvrnormArma(1, gam_mujm, gam_Sigjm).t();
...
// gam is (J, K-1, R)
// logW is (n, K)
void PMC::getLogWs(mat& logW, const cube& gam) {
...
void PMC::getLogWsSmall(mat& logWsmall, const cube& gam) {
    // Rcout << "start of PMC::getLogWs()" << endl;
    logWsmall.zeros();
    if (K==1)  return;

    // Compute eta.
    mat eta(logWsmall.n_rows, K-1);
    umat psiX_small = CpsiX_small.tail_cols(R);
    for (size_t j=0; j<J; j++) {
        uvec C_j = arma::find(CpsiX_small.col(0)==j);
        mat jGam = gam.row(j);  // (K-1, R)
        if (R==1)  eta.rows(C_j) = psiX_small.rows(C_j) * jGam;  // (n_j, K-1)
        else  eta.rows(C_j) = psiX_small.rows(C_j) * jGam.t();  // (n_j, K-1)
    }
    ...
    if (treestr == 1) {  // BT
        ...
        for (size_t k=0; k<K; k++) {
            size_t l=k+K;  // root=1 index scheme
            while (l > 1) {
                int s = (l%2 == 0 ? -1 : 1);  // sign: want odd l to be 1, even l to be -1
                l /= 2;
                logWsmall.col(k) -= log(1 + exp( s*eta.col(l-1) ));
            }
        }

    } else if (treestr == 0) {  // LT

        logWsmall.tail_cols(K-1) = cumsum(-log(1 + exp(eta)), 1);
        logWsmall.head_cols(K-1) -= log(1 + exp(-eta));
...
void PMC::get_chain_v2(Rcpp::List& chain, bool to_clr_saves)
{
    ...
    Rcout << "saveWsmall: sum of weights per row and pd" << endl;
    Rcout << saveWsmall.n_rows << " " << saveWsmall.n_cols << " " << saveWsmall.n_slices;
    if (false) {
        for (size_t r=0; r<J*pow(2,R-1); r++) {
            Rcout << endl << "-----  r=" << r;
            for (size_t pd=0; pd<length_chain; pd++) {
                Rcout << " " << accu(saveWsmall.slice(pd).row(r));
            }
        }
    }
    ...
    chain["Wsmall"] = saveWsmall;
    if (to_clr_saves)  saveWsmall.reset();
    chain["xi"] = saveXi;
    ...
    chain["gamma"] = saveGam;
    if (to_clr_saves)  saveGam.reset();
    Rcout << "end of get_chain_v2()" << endl;
